...
To optimize the control variable, we write every $q\in Q_d$, by using a fixed basis $\Phi=\{\phi_1,\dotsc,\phi_{N_b}\}$ with $\phi_1,\dotsc,\phi_{N_b}\in H$ and scalars $q_1^0,q_1^1,\dotsc,q_1^M,\dotsc,q_{N_b}^0,q_{N_b}^1\dotsc,q_{N_b}^M\in\mathbb{R}$, as
\begin{equation}
\label{discrContrVar}
