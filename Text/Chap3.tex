...
\If{$k=0$}
\State \text{Compute the initial covariance matrix }$\mathbf{C}_{\mathbf{q}_0}^0$\text{ like defined with }$\sigma^2,\rho$
\Else
\State \text{Compute the covariance matrix }$\mathbf{C}_{\mathbf{q}_{k}}^{k}\text{ like defined with }\mathbf{q}_k,F_k,T_k,\mathbf{C}_{\mathbf{q}_{k-1}}^{k-1}$
\EndIf
...
\State \text{Compute vector }$\mathbf{C}_{\mathbf{q}_{k},F}^{k}$\text{ and store values }$\{F(\mathbf{q}_{k,j})\}_{j=1}^N$
...
\begin{algorithmic}[1]
\Procedure{FOM-EnOpt}{$\mathbf{q}_0,N,\varepsilon,k^*,\beta,r,\nu^*$}
