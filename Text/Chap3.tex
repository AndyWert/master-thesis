...
\caption{EnOpt algorithm}
\begin{algorithmic}[1]
\Function{EnOpt}{$F,\mathbf{q}_0,N,\varepsilon,k^*,\beta_1,\beta_2,r,\nu^*,\sigma^2,\rho,\textproc{pr} \gets \mathbf{lambda}\:\mathrm{mu}: \mathrm{mu}, \mathbf{C}_\mathrm{init}\gets\mathrm{None}$}
...
\caption{OptStep algorithm}
\begin{algorithmic}[1]
\Function{OptStep}{$F,\mathbf{q}_k,N,k,T_k,\mathbf{C}_k,F_k,\beta_1,\beta_2,r,\varepsilon,\nu^*,\sigma^2,\rho,\textproc{pr} \gets \mathbf{lambda}\:\mathrm{mu}: \mathrm{mu}$}
...
\While{$\Call{np.min}{\mathbf{C}_\mathrm{diag}+\beta^\mathrm{iter}_2*\mathbf{d}_k}<0$}
...
\caption{Line search}
\begin{algorithmic}[1]
\Function{LineSearch}{$F, \mathbf{q}_k, F_k, \mathbf{d}_k, \beta_1, r, \varepsilon, \nu^*, \textproc{pr} \gets \mathbf{lambda}\:\mathrm{mu}: \mathrm{mu}$}
